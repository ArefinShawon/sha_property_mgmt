...
class PropertySale(models.Model):
    ...
    property_id = fields.Many2one('property.property', required=True, domain="[('state', '=', 'available')]",
                                  string="Property Name", help='The property to be sold')
    partner_id = fields.Many2one('res.partner', string="Customer", required=True,
                                 help='The customer who is buying the property')
    order_date = fields.Date(string="Order Date", help='The order date of property')
    state = fields.Selection([('draft', 'Draft'),
                              ('invisible', ''),
                              ('confirm', 'Confirm')], default='draft', string="State", tracking=True,
                             domain=lambda self: [('state', '!=', 'invisible')])
    invoice_id = fields.Many2one('account.move', readonly=True, string="Invoice",
                                 help='The invoice reference for the property')
    invoiced = fields.Boolean(string='Invoiced', help='Is the property sale invoiced')
    billed = fields.Boolean(string='Commission Billed', help='Is the commission given for this property sale')
    sale_price = fields.Monetary(string="Sale Price", readonly=False, related='property_id.unit_price',
                                 help='The price of the property')
    any_broker = fields.Boolean(string='Any Broker', help="Enable if this sale have a Broker")
    broker_id = fields.Many2one('res.users', related='property_id.responsible_id',string="Salesperson", help='The Salesperson for this property sale')
    commission_plan_id = fields.Many2one('property.commission', string="Commission Plan",
                                         help="Select the Commission Plan for the broker")
    commission_type = fields.Char(compute='_compute_commission_and_commission_type', string="Commission Type",
                                  help='The type of the commission')
    commission = fields.Monetary(string='Commission', compute='_compute_commission_and_commission_type',
        ...
        return {
            ...
            'context': {
                ...
                'default_invoice_line_ids': [fields.Command.create({
                    'name': self.property_id.plot_name,
    ...
    def action_confirm(self):
        if self.partner_id.blacklisted:
            raise ValidationError(
                _('The Customer %r is Blacklisted.', self.partner_id.plot_name))
        self.state = 'confirm'
